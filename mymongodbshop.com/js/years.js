    ...
    function(data){ //в data находятся альбомы нужного года
      var outTmp = '';
			...
			for (var key in data) { //выводим на страницу полученные альбомы
				...
				outTmp += `<p class="name">${alb[1]}</p>`;
				outTmp += `<p class="album">` + '"' + `${alb[2].albumName}`+ '"' + `</p>`;
				//outTmp += `<p class="year">` + 'Year:' + `${alb[2].year}</p>`;
				var songs = alb[2].songs;
				...
				outTmp += '</select>';
				outTmp += `<div class="cost">${alb[2].cost}` + ' Rub' + `</div>`;
				outTmp += `<button class="add-to-cart" data-id="${alb[2].idAlbum}" art-id="${alb[0].$id}">Buy</button>`;
				outTmp += '</div>';	
			}
			outTmp += '</div>';
