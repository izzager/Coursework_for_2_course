...
use GraphAware\Neo4j\Client\ClientBuilder; //подключаем библиотеку для работы с Neo4j
	...
	$query = 'MATCH (n:Artist)-[:SING]->(nalbum) WHERE n.genre = "' . $_POST['genre'] . '" RETURN id(n) as idArtist, id(nalbum) as idAlbum, n.genre as genre, n.name as artist,nalbum.name as albumName, nalbum.year as year, nalbum.cost as cost, nalbum.songs as songs'; //делаю запрос на языке Cypher
	$result = $client->run($query);//посылаю запрос на базу данных и получаю результат
	//в качестве результата получаем все альбомы с их артистами, у которых жанр равен жанру из POST-запроса
	$cnt = 0; //счетчик
