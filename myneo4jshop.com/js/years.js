...
//получаем данные из БД (отсортированный массив годов) и вызываем функцию, показывающую их
function init() { 
    $.post(
        "core/startdb.php",
        {
...
function showYears(data) {
  data = JSON.parse(data); //получаем все годы из базы данных
  console.log(data); //выводим в консоль
  var out = ''; //в эту переменную будем складывать html-код
	for (var year in data) {            //выводим каждый год и для каждого года кнопку, 
		out += '<div class="year">';      //при нажатии на которую будет выдаваться список альбомов этого года
		out += `<p  style="font-size: 150%; font-family: Courier; text-align: center; font-weight: bold">${data[year]}</p>`;
		out += `<details class="year-name ${data[year]}" year-id="${data[year]}" ><summary>Show albums</summary>`;
		out += '</details>';
		out += '</div>';
	}
  $('.goods-out').html(out); //выводим на страницу
	$('.year-name').on('click', showAlbumOfOneYear); //показывается список альбомов этого года
	$('.add-to-cart').on('click', addToCart); //реакция на нажатие кнопки
}


function showAlbumOfOneYear() {
	var idY = $(this).attr('year-id'); //узнаем, на какой год было нажатие
	console.log(idY);
	$.post(                   //отправляю запрос с номером года
    "core/years.php",
    {
			"year": idY //номер года
    },
    function(data){ //в data находятся альбомы нужного года
			var outTmp = '';
			outTmp += '<div class="detail"><p> </p>'; //<p> </p> нужно для css-разметки
			data = JSON.parse(data);
			console.log(data);
			for (var key in data) { //выводим на страницу полученные альбомы
				outTmp += '<div class="cart">';
				outTmp += `<p class="name">${data[key].artist}</p>`;
				outTmp += `<p class="album">` + '"' + `${data[key].albumName}`+ '"' + `</p>`;
				var songs = data[key].songs;
				...
				outTmp += '</select>';
				outTmp += `<div class="cost">${data[key].cost}` + ' Rub' + `</div>`;
				outTmp += `<button class="add-to-cart" data-id="${data[key].idAlbum}" art-id="${data[key].idArtist}">Buy</button>`;
				outTmp += '</div>';	
			}
			outTmp += '</div>';
			$('.year-name').html(outTmp); //показываем альбомы
			$('.year-name').each(function(){ //удаляем альбомы из тех полей, где они не должны быть
				if ($(this).attr('year-id') != idY) { 
