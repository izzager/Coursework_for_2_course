...
function showGoods(data) {
  ...
  for (var key in data) { //просматриваем каждый альбом
    out += '<div class="cart">';
    out += `<p class="name">${data[key].artist}</p>`;
    out += `<p class="album">` + '"' + `${data[key].albumName}`+ '"' + `</p>`;
    out += `<p class="year">` + 'Year:' + `${data[key].year}</p>`;
    ...
    out += `<button class="add-to-cart" data-id="${data[key].idAlbum}" art-id="${data[key].idArtist}">Buy</button>`;
    out += '</div>';
  }
